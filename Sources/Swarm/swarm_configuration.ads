...
-----------------------------------------------------------------------------
   ...
   Charging_Setup : constant Charging_Setups_R :=
     Charging_Setups (Configuration);
   Energy_Globe_Detection : constant Real :=
     Energy_Globe_Detections (Configuration);
   Energy_Globes_Defaults : constant Energy_Globes :=
     Default_Globes (Configuration);
   Energy_Globes_Velocity : constant Vector_3D :=
     (x => 0.15, y => 0.0, z => 0.0);

   -- Orbiting parameters for globes
   Sphere_Increment : constant Vector_3D :=
     (x => 2.0 * Pi / Intented_Framerate / 1_000.0,
      y => 2.0 * Pi / Intented_Framerate / 1_100.0,
      z => 2.0 * Pi / Intented_Framerate / 120.0);

   Sphere_Increment_Fast : constant Vector_3D :=
     (x => 2.0 * Pi / Intented_Framerate / 100.0,
      y => 2.0 * Pi / Intented_Framerate / 110.0,
      z => 2.0 * Pi / Intented_Framerate / 12.0);

   Tolerated_Vehicle_Activation_Delay : constant Duration :=
     To_Duration (Milliseconds (100));
   Tolerated_Identify_Call_Delay : constant Duration :=
     To_Duration (Milliseconds (100));
   ...
   function Approach_Acceleration
     (x : Distances; Velocity_Towards_Goal : Real) return Acc_Scalar;
